...
/*
 * Deliberately absent: the merchant portal's registerOnMerchantInactive
 * hook (a 403 "merchant_inactive" meaning a tenant was suspended
 * mid-session). The platform admin is the operator above every tenant, not
 * a tenant itself — there is nothing here that can be suspended out from
 * under the session, so that hook would be permanently dead code. A 403 in
 * this app means a role check failed, which is handled per-request by the
 * caller, not globally.
 */
